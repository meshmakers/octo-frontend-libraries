import { applyEntitySelector, isNavigationPath, parseEntitySelector, stripEntitySelector, } from './entity-selector-path';

describe('entity-selector-path', () => {
  const basePath = 'containedSensors.energyIQTemperatureSensor->currentValue';
  const selectorPath = "containedSensors.energyIQTemperatureSensor[wellKnownName='Sensor 1']->currentValue";

  describe('isNavigationPath', () => {
    it('detects navigation paths', () => {
      expect(isNavigationPath(basePath)).toBe(true);
      expect(isNavigationPath('currentValue')).toBe(false);
      expect(isNavigationPath('amount.value')).toBe(false);
    });
  });
  ...
  describe('parseEntitySelector', () => {
    ...
    it('parses rtId selectors', () => {
      const selector = parseEntitySelector('containedSensors.energyIQTemperatureSensor[rtId=6789a00000000000010012a1]->currentValue');
      expect(selector).toEqual({ kind: 'rtId', value: '6789a00000000000010012a1' });
    });

    it('parses attribute selectors', () => {
      const selector = parseEntitySelector('containedSensors.energyIQTemperatureSensor[name=Main]->currentValue');
  ...
  describe('applyEntitySelector', () => {
    it('inserts a selector on the first navigation segment', () => {
      expect(applyEntitySelector(basePath, { kind: 'wellKnownName', value: 'Sensor 1' })).toBe(selectorPath);
    });

    it('replaces an existing selector', () => {
      expect(applyEntitySelector(selectorPath, { kind: 'rtId', value: 'abc' })).toBe("containedSensors.energyIQTemperatureSensor[rtId='abc']->currentValue");
    });

    it('applies attribute selectors with the attribute name as key', () => {
      expect(applyEntitySelector(basePath, {
        kind: 'attribute',
        attributeName: 'name',
        value: 'Main',
    ...
    it('targets the first navigation segment on multi-hop paths', () => {
      expect(applyEntitySelector('children.testSensor->parent.testZone->designation', {
        kind: 'wellKnownName',
        value: 'S1',
    ...
    it('is a no-op on non-navigation paths', () => {
      expect(applyEntitySelector('currentValue', { kind: 'rtId', value: 'x' })).toBe('currentValue');
    });
  });
});
