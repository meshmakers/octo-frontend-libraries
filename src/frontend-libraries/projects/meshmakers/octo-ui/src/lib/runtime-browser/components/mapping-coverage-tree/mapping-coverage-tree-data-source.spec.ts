...
function coverageNodeResponse(
  rtId: string,
  ckTypeId: string,
  name: string,
  ownMappings: number,
  children: CoverageChildStub[],
) {
  return {
    data: {
      runtime: {
...
function entityItem(payload: Partial<CoverageNodePayload> & { rtId: string; ckTypeId: string }) {
  ...
  return new TreeItemDataTyped<CoverageNodePayload>(
    full.rtId, full.name, '', full, gridIcon, true, false,
  );
}

describe('MappingCoverageTreeDataSource (association auto-discovery)', () => {
  let dataSource: MappingCoverageTreeDataSource;
  let coverageNodeGQL: jasmine.SpyObj<GetMappingCoverageNodeDtoGQL>;
  let associationsGQL: jasmine.SpyObj<GetRuntimeEntityAssociationsByIdDtoGQL>;
  let rolesGQL: jasmine.SpyObj<GetCkTypeAssociationRolesDtoGQL>;
  let treeNavConfig: jasmine.SpyObj<TreeNavigationConfigService>;

  beforeEach(() => {
    coverageNodeGQL = jasmine.createSpyObj('GetMappingCoverageNodeDtoGQL', ['fetch']);
    associationsGQL = jasmine.createSpyObj('GetRuntimeEntityAssociationsByIdDtoGQL', ['fetch']);
    rolesGQL = jasmine.createSpyObj('GetCkTypeAssociationRolesDtoGQL', ['fetch']);
    treeNavConfig = jasmine.createSpyObj('TreeNavigationConfigService', ['resolve']);
  ...
  function arrangeSpaceWithSensors(): void {
    coverageNodeGQL.fetch.and.callFake(((options: {
      variables?: { childRoleId?: string };
    }) => {
      if (options.variables?.childRoleId === 'EnergyIQ/SpaceSensors') {
        return of(
          coverageNodeResponse('space1', 'EnergyIQ/Space', 'Space 1', 0, [
      ...
      return of(
        coverageNodeResponse('space1', 'EnergyIQ/Space', 'Space 1', 1, [
          { rtId: 'childSpace', ckTypeId: 'EnergyIQ/Space', name: 'Child Space', grandChildren: 1 },
        ]),
      );
    }) as never);
    associationsGQL.fetch.and.returnValue(
      of(
    ...
    rolesGQL.fetch.and.callFake(((options: { variables?: { ckTypeId?: string } }) => {
  ...
  it('adds a group node per discovered role next to the spatial children', async () => {
    ...
    const children = await dataSource.fetchChildren(
      entityItem({ rtId: 'space1', ckTypeId: 'EnergyIQ/Space', name: 'Space 1' }),
    );
  ...
  it('expands a group node into its target entities with mapping counts', async () => {
    ...
    const lastVariables = coverageNodeGQL.fetch.calls.mostRecent().args[0]?.variables;
  ...
  it('honours TreeNavigationConfiguration overrides (hide + relabel)', async () => {
    arrangeSpaceWithSensors();
    treeNavConfig.resolve.and.callFake((_sourceCkTypeId: string, roleId: string) =>
      Promise.resolve(
    ...
    treeNavConfig.resolve.and.resolveTo({ visible: false });
    const hidden = await dataSource.fetchChildren(
      entityItem({ rtId: 'space1', ckTypeId: 'EnergyIQ/Space', name: 'Space 1' }),
    );
  ...
  it('flattens a role when its override sets grouped: false', async () => {
    ...
    treeNavConfig.resolve.and.callFake((_sourceCkTypeId: string, roleId: string) =>
      Promise.resolve(
        roleId === 'EnergyIQ/SpaceSensors' ? { grouped: false } : undefined,
      ),
    );
    ...
    expect(children.every(c => !c.item.associationGroup)).toBeTrue();
  ...
  it('marks children without spatial grandchildren expandable when their type has navigable inbound roles', async () => {
    ...
    expect(targets[0].expandable).toBeFalse();
    // The spatial child has grandchildren → expandable regardless of schema.
    expect(children[0].expandable).toBeTrue();
  });
});
