...
import { AttributeMapperService, BINARY_LINKED_REFERENCE_FLAG, } from "./attribute-mapper.service";
...
describe("AttributeMapperService", () => {
    ...
    let resolverSpy: MockedObject<AttributeMetadataResolverService>;

    /** Builds a top-level Attribute matching what AttributeDataService produces for the form. */
    function attr(name: string, type: string, optional = false, ckId = ""): Attribute {
        return {
            id: { ckId, rtId: null },
            attributeName: name,
    ...
    function ckMeta(name: string, type: string, optional = false, ckAttributeFullName?: string): CkAttributeMetadata {
        return {
            attributeName: name,
            attributeValueType: type,
            isOptional: optional,
    ...
    describe("mapFormValueToGraphQLAttributes — top-level", () => {
        ...
        it("emits scalar attributes verbatim when value is present", async () => {
            const result = await service.mapFormValueToGraphQLAttributes({ name: "Acme", count: 3 }, [attr("name", "STRING"), attr("count", "INTEGER")]);
        ...
        it("skips required scalar when value is null/undefined", async () => {
            const result = await service.mapFormValueToGraphQLAttributes({ name: null, count: undefined }, [attr("name", "STRING"), attr("count", "INTEGER")]);
            expect(result).toEqual([]);
        });

        it("emits explicit null for optional scalar that was cleared", async () => {
            const result = await service.mapFormValueToGraphQLAttributes({ description: null }, [attr("description", "STRING", true)]);
            expect(result).toEqual([{ attributeName: "description", value: null }]);
        });

        it("converts GEOSPATIAL_POINT to GeoJSON shape", async () => {
            const result = await service.mapFormValueToGraphQLAttributes({ location: { longitude: 19.94, latitude: 50.06 } }, [attr("location", "GEOSPATIAL_POINT")]);
            expect(result).toEqual([
                {
        ...
        it("converts TIME_SPAN Date to total seconds", async () => {
            ...
            const result = await service.mapFormValueToGraphQLAttributes({ duration: time }, [attr("duration", "TIME_SPAN")]);
        ...
        it("normalizes INTEGER_ARRAY values from {key} wrappers to primitives", async () => {
            const result = await service.mapFormValueToGraphQLAttributes({ codes: [{ key: 1 }, { key: "2" }, 3] }, [attr("codes", "INTEGER_ARRAY")]);
    ...
    describe("BINARY / BINARY_LINKED handling", () => {
        it("converts BINARY File to a byte array", async () => {
            ...
            const result = await service.mapFormValueToGraphQLAttributes({ payload: [file] }, [attr("payload", "BINARY")]);
        ...
        it("preserves BINARY_LINKED File as-is (multipart upload path)", async () => {
            ...
            const result = await service.mapFormValueToGraphQLAttributes({ document: [file] }, [attr("document", "BINARY_LINKED")]);
        ...
        it("skips synthetic BINARY_LINKED reference Files (no re-upload)", async () => {
            ...
            (reference as unknown as Record<string, unknown>)[BINARY_LINKED_REFERENCE_FLAG] = true;
            const result = await service.mapFormValueToGraphQLAttributes({ document: [reference] }, [attr("document", "BINARY_LINKED")]);
            expect(result).toEqual([]);
        });

        it("emits null for cleared BINARY_LINKED so the backend detaches the link", async () => {
            const result = await service.mapFormValueToGraphQLAttributes({ document: [] }, [attr("document", "BINARY_LINKED")]);
        ...
        it("throws on malformed BINARY_LINKED base64 instead of silently nulling the value", async () => {
            ...
            await expect(service.mapFormValueToGraphQLAttributes({ document: "###not-base64###" }, [attr("document", "BINARY_LINKED")])).rejects.toThrowError(/BINARY_LINKED/);
        });
    });
    ...
    describe("RECORD / RECORD_ARRAY recursive mapping", () => {
        it("emits a flat object for RECORD value (not the {attributes:[…]} wrapper)", async () => {
            ...
            const result = await service.mapFormValueToGraphQLAttributes({ address: { street: "Main", zipcode: 1 } }, [attr("address", "RECORD", false, "Basic/Address-1")]);

            expect(resolverSpy.getRawAttributes$).toHaveBeenCalledWith("Basic/Address-1", true);
            expect(result).toEqual([
                {
        ...
        it("emits an array of flat objects for RECORD_ARRAY value", async () => {
            resolverSpy.getRawAttributes$.mockReturnValue(of([ckMeta("name", "STRING")] as CkAttributeMetadata[]));

            const result = await service.mapFormValueToGraphQLAttributes({ tags: [{ name: "a" }, { name: "b" }] }, [attr("tags", "RECORD_ARRAY", false, "Basic/Tag-1")]);
        ...
        it("preserves camelCase sub-attribute names on the wire", async () => {
            ...
            const result = await service.mapFormValueToGraphQLAttributes({ namePlate: { manufacturerName: "X", vATnumber: "PL000" } }, [attr("namePlate", "RECORD", false, "Basic/NamePlate-1")]);
        ...
        it("falls back to empty sub-metadata when resolver throws (no crash, sub-values pass through)", async () => {
            resolverSpy.getRawAttributes$.mockReturnValue(throwError(() => new Error("network down")));
            vi.spyOn(console, "error").mockReturnValue(undefined); // suppress expected error log

            const result = await service.mapFormValueToGraphQLAttributes({ address: { street: "Main" } }, [attr("address", "RECORD", false, "Basic/Address-1")]);
        ...
        it("passes record values through verbatim when recordCkId is missing", async () => {
            const result = await service.mapFormValueToGraphQLAttributes({ address: { street: "Main" } }, [attr("address", "RECORD", false, "")]);
        ...
        it("preserves explicit null sub-values in pass-through mode (cleared nested fields)", async () => {
            ...
            const result = await service.mapFormValueToGraphQLAttributes({ address: { street: null, city: "Berlin" } }, [attr("address", "RECORD", false, "Basic/Address-1")]);
