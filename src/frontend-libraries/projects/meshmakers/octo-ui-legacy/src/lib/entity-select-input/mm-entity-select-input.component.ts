/**
 * Material Design replacement for legacy mm-entity-select from @meshmakers/shared-ui.
 * Uses Angular Material Autocomplete with ControlValueAccessor.
 */
import {
  Component,
  DoCheck,
...
@Component({
  ...
  template: `
    <input #input (blur)="onTouched()" (focusout)="onFocusOut()" [formControl]="searchFormControl"
      [matAutocomplete]="auto" [placeholder]="placeholder" matInput>
  ...
  `,
  styles: [`
    span { opacity: 0; transition: opacity 200ms; }
    :host.floating span { opacity: 1; }
  `],
  host: {
    '[id]': 'id',
        ...
        .subscribe(async (value: string) => {
          ...
          if (resultSet.items) {
            if (resultSet.items.length === 1) {
              this.value = resultSet.items[0];
            } else {
              this.filteredEntities = resultSet.items;
            }
          }
          this.isLoading = false;
        });
