...
describe('AiNewAppDialogComponent', () => {
  ...
  const templates: readonly AiAppTemplateDto[] = [
    {
      ...
      jobKind: 'Application',
      goalTemplate:
        'Generate a CRUD list+detail app for {{primaryEntity}}. Project name: {{projectName}}.',
  ...
  it('keeps Submit disabled until a template is picked AND every required field is filled', () => {
    const submitButton: HTMLButtonElement = fixture.nativeElement.querySelector(
      '.mm-ai-new-app__btn--submit',
    );
    expect(submitButton.disabled).toBeTrue();

    // Pick the first template — but no fields filled yet.
    const firstTemplateRadio: HTMLInputElement =
      fixture.nativeElement.querySelectorAll(
        '.mm-ai-new-app__template input[type="radio"]',
      )[0];
  ...
  it('previews the rendered Goal once every placeholder has a value', () => {
    ...
    expect(preview.textContent.trim()).toBe(
      'Generate a CRUD list+detail app for Customer. Project name: customer-list.',
    );
  });
